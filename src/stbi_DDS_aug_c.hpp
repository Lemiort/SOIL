...
void stbi_decode_DXT1_block(unsigned char uncompressed[16 * 4],
...
void stbi_decode_DXT23_alpha_block(unsigned char uncompressed[16 * 4],
                                   unsigned char compressed[8]) {
    int i, next_bit = 0;
    //	each alpha value gets 4 bits
    for (i = 3; i < 16 * 4; i += 4) {
        uncompressed[i] = stbi_convert_bit_range(
            (compressed[next_bit >> 3] >> (next_bit & 7)) & 15, 4, 8);
        next_bit += 4;
    }
}
void stbi_decode_DXT45_alpha_block(unsigned char uncompressed[16 * 4],
                                   unsigned char compressed[8]) {
    int i, next_bit = 8 * 2;
...
void stbi_decode_DXT_color_block(unsigned char uncompressed[16 * 4],
                                 unsigned char compressed[8]) {
    int next_bit = 4 * 8;
    int i, r, g, b;
    int c0, c1;
...
static stbi_uc *dds_load(stbi *s, int *x, int *y, int *comp, int req_comp) {
    ...
    has_alpha =
        (header.sPixelFormat.dwFlags & DDPF_ALPHAPIXELS) / DDPF_ALPHAPIXELS;
    has_mipmap =
        (header.sCaps.dwCaps1 & DDSCAPS_MIPMAP) && (header.dwMipMapCount > 1);
    cubemap_faces =
        (header.sCaps.dwCaps2 & DDSCAPS2_CUBEMAP) / DDSCAPS2_CUBEMAP;
    ...
    *comp = s->img_n;
    ...
    if (is_compressed) {
        /*	compressed	*/
        //	note: header.sPixelFormat.dwFourCC is something like
        //(('D'<<0)|('X'<<8)|('T'<<16)|('1'<<24))
        ...
        for (cf = 0; cf < cubemap_faces; ++cf) {
            ...
            for (i = 0; i < num_blocks; ++i) {
                ...
                //	get the next block's worth of compressed data, and
                // decompress it
                if (DXT_family == 1) {
                    //	DXT1
                    getn(s, compressed, 8);
                ...
                for (by = 0; by < bh; ++by) {
                    int idx =
                        4 * ((ref_y + by + cf * s->img_x) * s->img_x + ref_x);
                    for (bx = 0; bx < bw * 4; ++bx) {
        ...
        *comp = s->img_n;
        ...
        for (cf = 0; cf < cubemap_faces; ++cf) {
            ...
            getn(s, &dds_data[cf * s->img_x * s->img_y * s->img_n],
    ...
    *y = s->img_y;
    ...
    if ((req_comp <= 4) && (req_comp >= 1)) {
        ...
        if (req_comp != s->img_n) {
            dds_data = convert_format(dds_data, s->img_n, req_comp, s->img_x,
                                      s->img_y);
            *comp = s->img_n;
        }
    } else {
...
#ifndef STBI_NO_STDIO
stbi_uc *stbi_dds_load_from_file(FILE *f, int *x, int *y, int *comp,
                                 int req_comp) {
    stbi s;
    start_file(&s, f);
...
stbi_uc *stbi_dds_load(char *filename, int *x, int *y, int *comp,
                       int req_comp) {
    stbi_uc *data;
...
stbi_uc *stbi_dds_load_from_memory(stbi_uc const *buffer, int len, int *x,
